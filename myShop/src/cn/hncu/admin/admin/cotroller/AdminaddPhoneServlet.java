...
public class AdminaddPhoneServlet extends HttpServlet {
...
//			error("上传的文件超出了80KB", request, response);
		...
		Phone phone = CommonUtils.toBean(map, Phone.class);//把Map中大部分数据封装到phone对象中
...
//		}
		...
		// 把图片的路径设置给book对象
		phone.setImage_w("book_img/" + filename);
...
//			error("上传的图片扩展名必须是JPG", request, response);
		...
		if(image.getWidth(null) > 350 || image.getHeight(null) > 350) {
//			error("您上传的图片尺寸超出了350*350！", request, response);
			System.out.println("图片超出大小，删除！");
			destFile.delete();//删除图片
			return;
		}
		
		// 把图片的路径设置给book对象
		phone.setImage_b("book_img/" + filename);
		
		
		// 调用service完成保存
		phone.setPid(CommonUtils.uuid());

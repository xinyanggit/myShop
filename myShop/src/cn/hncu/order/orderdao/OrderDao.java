...
public class OrderDao implements IOrderDao {
		...
		public void add(Order order) throws SQLException {
			...
			for(int i = 0; i < len; i++){
				...
				objs[i] = new Object[]{item.getOrderItemId(),item.getQuantity(),
						item.getSubtotal(),item.getPhone().getPid(),
						item.getPhone().getPname(),item.getPhone().getCurrprice(),
						item.getPhone().getImage_b(),order.getOid()};
			}
			qr.batch(sql, objs);
		}
		...
		private PageBean<Order> findByCriteria(List<Expression> exprList, int pc) throws SQLException {
			 ...
			 * 1. 得到ps
			 */
			int ps = PageConstants.ORDER_PAGE_SIZE;//每页记录数 写死
			/*

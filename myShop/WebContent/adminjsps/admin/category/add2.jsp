...
<style type="text/css">
  ...
  <body>
    ...
    <form action="<c:url value='/sp/admin/addChild'/>" method="post" onsubmit="return checkForm()">
    	分类名称：<input type="text" name="cname" id="cname"/><br/>
    	一级分类：<select name="parentid" id="parentid">
    		<option value="">===选择1级分类===</option>
				<c:forEach items="${parents }" var="parent">
				    		<option value="${parent.cid }" <c:if test="${parent.cid eq parentid }">selected="selected"</c:if> >${parent.cname }</option>
				</c:forEach>
    	</select>
    	<br/>
